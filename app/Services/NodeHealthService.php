...
final class NodeHealthService
{
    ...
    public function poll(Node $node): void
    {
        try {
            ...
            $healthyStatus = $node->status === NodeStatus::Ready ? NodeStatus::Ready : NodeStatus::Online;
            $node->update(['status' => $node->maintenance ? NodeStatus::Maintenance : $healthyStatus, 'last_health_status' => $health['status'] ?? 'ok', 'agent_version' => $health['agent_version'] ?? $health['version'] ?? null, 'capabilities' => $capabilities, 'cpu_count' => $resources['cpu_count'] ?? 0, 'memory_total_bytes' => $resources['memory_total_bytes'] ?? 0, 'memory_available_bytes' => $resources['memory_available_bytes'] ?? 0, 'disk_total_bytes' => $resources['disk_total_bytes'] ?? 0, 'disk_available_bytes' => $resources['disk_available_bytes'] ?? 0, 'deployment_count' => $resources['deployment_count'] ?? 0, 'active_deployment_count' => $resources['active_deployment_count'] ?? 0, 'last_seen_at' => now(), 'last_error_code' => null]);
